  ...
  const fetchTotalBlogs = useCallback(async () => {
    try {
      const response: Response = await fetch(
        "http://localhost:3001/api/blogs/count"
      );
        ...
        <div className="bg-gray-50 col-span-2 p-4 rounded">
          ...
          {result.length > 0 ? (
            <div className="flex justify-center mt-4 space-x-8">
              <button
                className="text-black/70 btn btn-ghost disabled:bg-inherit focus:outline-none"
                onClick={prevPage}
                disabled={currentPage === 1}
              >
                Previous
              </button>
              <span className="text-black/70 my-3 font-mono">
                Page {currentPage} of {Math.ceil(totalBlogs / itemsPerPage)}
              </span>
              <button
                className="text-black/70 btn btn-ghost disabled:bg-inherit focus:outline-none"
                onClick={nextpage}
